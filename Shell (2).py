...
class Shell(Cmd):
    ...
    def ficheiro(self):
        ...
        for i in BDS.BD:
            print(i)
            seq=''.join(BDS.BD[i])
            seq_insert=MySeq(seq,'protein')
            if seq_insert.valida():
                dict[i] = {'seq': seq_insert}
                dictionary = {'tamanho': seq_insert.__len__(), 'maior proteína': seq_insert.maiorProteina()}
                dict[i].update(dictionary)
